...
function extract_search_params (sns_message_raw) {
  ...
  var obj = {
    ...
    children: 0,
    searchId: msg.id,
    id: msg.data.context.connectionId,
    userId: msg.data.context.userId
  };
  ...
  if (msg.data.content && msg.data.content.hotels && msg.data.content.hotels.length > 0) {
    AwsHelper.log.info({ query: msg.data.query }, 'Query from SNS');
    obj.hotelIds = msg.data.content.hotels.map(function (h) {
  ...
  // if (msg.data.query.travelPeriod && msg.data.query.travelPeriod.departureBetween) {
  //   obj.departureDate = msg.data.query.travelPeriod.departureBetween[0];
  // }
  // departureAirports
  if (msg.data.query.departureAirports && msg.data.query.departureAirports[0].indexOf('.') > -1) {
    obj.departureCode = msg.data.query.departureAirports[0].split('.')[1];
  }
  // number of nights
  if (msg.data.query.travelPeriod && msg.data.query.travelPeriod.nights) {
    obj.hotelDuration = msg.data.query.travelPeriod.nights[0] + 1;
  }
  return obj;
}
