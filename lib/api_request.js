  ...
  if (params.hotelIds && params.hotelIds.length > 1) {
    ...
    hids.forEach(function (hid, index) { // send *many* queryies to API Gateway Cache
      ...
      if (CACHE[options.path]) {
        // console.log('CACHE COUNT:', Object.keys(CACHE).length);
        ...
        http_request(options, function (err, data) {
          ...
          if (!err && data.result && data.result.length > 0) {
            ...
            if (options.path.indexOf('hotels') > -1) {
              CACHE[options.path] = result; // hotel info rarely chances
            }
            results.result.push(result);
            results.totalHits += data.totalHits;
          } else {
            CACHE[options.path] = {}; // avoid re-calling API when we Know there are no results
          }
          if (--countdown === 0) {
            AwsHelper.log.info({ 'results': log }, 'Package Results');
