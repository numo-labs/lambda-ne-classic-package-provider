...
module.exports = function api_request (params, callback) {
  ...
  function callApi () {
    ...
    if (params.hotelIds && params.hotelIds.length > 0) {
        ...
        function iteratee (hid, done) {
          ...
          http_request(options, function (err, data) {
            if (!err && data.result && Array.isArray(data.result) && data.result.length > 0) {
              ...
              AwsHelper.log.info({ delta: Date.now() - startedAt }, 'Amount of time to fetch packages');
              ...
              get_hotel_info(hid, function (err, hotel_info) {
                ...
                AwsHelper.log.info({ delta: Date.now() - startedAt2 }, 'Amount of time to fetch hotel info');
      ...
      http_request(options, function (err, data) {
        if (err || !data.result || !Array.isArray(data.result) || data.result.length === 0) {
          var err2 = err || new Error('No hotels package results');
          AwsHelper.log.info({err: err2}, 'No Hotels Package results');
