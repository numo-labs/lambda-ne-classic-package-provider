...
module.exports = function api_request (params, callback) {
  ...
  if (params.hotelIds && params.hotelIds.length > 1) {
    ...
    hids.forEach(function (hid, index) { // send *many* queryies to API Gateway Cache
      var _params = JSON.parse(JSON.stringify(params)); // clone
      _params.hotelIds = hid;
      var options = make_options(stage, _params);
      // we cache the hotel info on the Lambda's Container RAM to save 400ms ...
      // it only takes up 40MB of RAM. Benefit far out-weighs cost.
      if (options.path.indexOf('hotels') > -1 && CACHE[hid]) {
        var result = CACHE[hid];
        ...
        http_request(options, function (err, data) {
          ...
          if (!err && data.result && data.result.length > 0) {
            ...
            if (options.path.indexOf('hotels') > -1) {
              CACHE[hid] = result; // hotel info rarely changes
            }
            results.result.push(result);
